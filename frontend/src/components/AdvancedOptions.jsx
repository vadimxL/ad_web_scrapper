...
// Advanced options component. Currently only sellerType is propagated upward.
export default function AdvancedOptions({ sellerType, setSellerType }) {
    const [adType, setAdType] = React.useState('all');          // סוג מודעות (local only for now)
    // sellerType is controlled by parent
    const [ownerType, setOwnerType] = React.useState('all');    // בעלים (local only for now)
